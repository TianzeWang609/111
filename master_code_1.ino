...
void course_correction() {
  right_motor ->setSpeed(158);  // Max speed for right motor
  left_motor ->setSpeed(150);  // Max speed for left motor
  if (leftVal == LOW && rightVal == LOW && frontrightVal == LOW && frontleftVal == LOW) {
    // all sensors see black (move forward)
    right_motor->run(FORWARD);  
    left_motor->run(FORWARD);  
    Serial.println("Driving forward");

  }else if (leftVal == LOW && rightVal == HIGH && frontleftVal == LOW && frontrightVal == LOW) {
  ...
  } else if (leftVal == HIGH && rightVal == LOW && frontrightVal == LOW && frontleftVal == LOW) {
