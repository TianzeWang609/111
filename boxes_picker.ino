...
// create object for time of flight and servo
DFRobot_VL53L0X TOF;
Servo gate_servo; 
Servo ramp_servo;

// create a variable for servo position for gate and ramp
int pos_gate = 0; 
int pos_ramp = 0;

// create a variable for the box distance roughly 
float box_distance = 10 ;
...
// create magnetic sensor pin
int magnetic_sensor_pin = 12;

// create a variable for a magnetic box in possesion 
bool box_is_magnetic =  false ;
...
void finding_box () {
  static bool is_box_collected = false ;
  gate_servo.write( pos_gate + 180 ); 
  if (TOF.getDistance() <= box_distance && !is_box_collected) {
    right_motor -> run(RELEASE) ;
    left_motor -> run(RELEASE);
    gate_servo.write(pos_gate) ;
    delay(100) ;
    ramp_servo.write( pos_ramp + 180);
    is_box_collected = true;
  }
}
...
void check_if_magnetic () {
  int val = 0 ;
  val = digitalRead(magnetic_sensor_pin); // read input value
  if (val == HIGH) { // check if the input is HIGH
    box_is_magnetic = true ;
  }
}
